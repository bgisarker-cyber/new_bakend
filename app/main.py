# app/main.py
from io import BytesIO
from typing import Optional
from fastapi import FastAPI, logger, status, HTTPException
from pydantic import BaseModel, EmailStr, constr
from datetime import date, datetime, time
from decimal import Decimal
import smtplib
from email.message import EmailMessage
from fastapi.middleware.cors import CORSMiddleware
import os
import traceback
from .routers import users
from app.database import conn, cursor
from app.routers import users
from app.local import debug, demo,store,faulty
from app.routers import citypos,pubalipos,islami,mtb,standard,pb_wittdraw,pb_replace,city_replace
from app.utils import logger
from app.tasks import tasks,uit_task




# ===================================================
# App init
# ===================================================
app = FastAPI()

# include routers
app.include_router(users.router)
app.include_router(citypos.router)
app.include_router(pubalipos.router)
app.include_router(pubalipos.router)
app.include_router(islami.router)
app.include_router(mtb.router)
app.include_router(standard.router)
app.include_router(demo.router)
app.include_router(logger.router)
app.include_router(debug.router)
app.include_router(store.router)
app.include_router(faulty.router)
app.include_router(pb_wittdraw.router)
app.include_router(pb_replace.router)
app.include_router(city_replace.router)
app.include_router(tasks.router)
app.include_router(uit_task.router)





#app.include_router(users.router, prefix="")  # routes defined in devices.py
  # routes defined in devices.py


app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000", "http://127.0.0.1:3000"],  # adjust or "*"
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

@app.get("/")
def read_root(status=status.HTTP_202_ACCEPTED):
    return {"message": " Welcome to BGI Inventory System"}





