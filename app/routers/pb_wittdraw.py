...
router = APIRouter(prefix="/pubali_withdraw", tags=["pubali_withdraw"])

# ================================
# Helper Function
# ================================
def _truncate(value: Any, max_len: int = 255):
    """Safely truncate and clean string values."""
...
def get_all_pubali_withdraw(current_user: dict = Depends(get_current_user)):
    check_role(current_user, ["support", "admin", "superadmin"])
    try:
        cursor.execute('SELECT * FROM pubali_withdraw ORDER BY "S/N" ASC;')
...
def edit_pubali_withdraw(sn: int, payload: Dict[str, Any] = {}, current_user: dict = Depends(get_current_user)):
    check_role(current_user, ["admin", "superadmin"])
    try:
        set_clause = ", ".join([f'"{k}" = %s' for k in payload.keys()])
        values = tuple(payload.values()) + (sn,)
        cursor.execute(f'UPDATE pubali_withdraw SET {set_clause} WHERE "S/N" = %s;', values)
        conn.commit()

        log_user_action(
            user_email=current_user["email"],
            user_role=current_user["role"],
            action="Edit Pubali Withdraw",
            target_table="pubali_withdraw",
...
def delete_pubali_withdraw(sn: int, current_user: dict = Depends(get_current_user)):
    check_role(current_user, ["admin", "superadmin"])
    try:
        cursor.execute('DELETE FROM pubali_withdraw WHERE "S/N" = %s;', (sn,))
        conn.commit()

        log_user_action(
            user_email=current_user["email"],
            user_role=current_user["role"],
            action="Delete Pubali Withdraw",
            target_table="pubali_withdraw",
...
def upload_pubali_withdraw_excel(file: UploadFile = File(...), current_user: dict = Depends(get_current_user)):
    check_role(current_user, ["admin", "superadmin"])
    try:
        df = pd.read_excel(BytesIO(file.file.read()),sheet_name=1)
        ...
        for _, row in df.iterrows():
            try:
                ...
                # Check existing
                cursor.execute('SELECT "S/N" FROM pubali_withdraw WHERE "POS S/N" = %s;', (pos_sn,))
                exists = cursor.fetchone()

                if exists:
                    set_clause = ", ".join([f'"{k}" = %s' for k in record])
                    cursor.execute(
                        f'UPDATE pubali_withdraw SET {set_clause} WHERE "POS S/N" = %s;',
                        (*record.values(), pos_sn)
                    )
                    updated += 1
                else:
                    ...
                    cursor.execute(
                        f'INSERT INTO pubali_withdraw ({columns}) VALUES ({placeholders});',
                        tuple(record.values())
                    )
                    inserted += 1
            except Exception as e:
        ...
        log_user_action(
            user_email=current_user["email"],
            user_role=current_user["role"],
            action="Bulk Upload Pubali Withdraw",
            target_table="pubali_withdraw",
...
# ================================
# 5️⃣ Export Excel (with Bearer Token)
# ================================
@router.get("/download", status_code=status.HTTP_200_OK)
def download_pubali_withdraw(current_user: dict = Depends(get_current_user)):
    check_role(current_user, ["support", "admin", "superadmin"])
    try:
        columns = [
            "S/N", "CONFIGURATION DATE", "TID", "MID", "MERCHANT NAME",
        ...
        cursor.execute(f'SELECT {", ".join(quoted)} FROM pubali_withdraw ORDER BY "S/N";')
        rows = cursor.fetchall()
        ...
        output.seek(0)
        headers = {"Content-Disposition": 'attachment; filename="pubali_withdraw_data.xlsx"'}
...
# ================================
@router.get("/template")
def download_pubali_withdraw_template():
    cols = [
        "S/N", "CONFIGURATION DATE", "TID", "MID", "MERCHANT NAME",
        "DBA NAME", "ADDRESS", "BANK CONT. PERSON NAME", "BANK CONT. PERSON NUMBER",
        "DISTRICT", "DIVISION", "ZONE", "POS S/N", "POS BRAND & MODEL",
    ...
    buf.seek(0)
    headers = {"Content-Disposition": 'attachment; filename="pubali_withdraw_template.xlsx"'}
