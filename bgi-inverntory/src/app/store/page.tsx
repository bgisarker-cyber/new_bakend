...
const StorePage = () => {
  ...
  const handleAddStore = async () => {
    ...
    try {
      ...
      const updated = await fetch("/api/store/all", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleUpdateStore = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/store/edit/${editingStore.sl}`,
        {
          method: "PUT",
          headers: getAuthHeaders(),
          body: JSON.stringify({
  ...
  const handleDelete = async (sl: number, pos_serial: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/store/delete/${sl}`, {
        method: "DELETE",
        headers: getAuthHeaders(),
        body: JSON.stringify({
          log_user_email: email,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          <div className="bg-[#e6e9ef] p-6 rounded-2xl w-full max-w-md shadow-[8px_8px_16px_rgba(0,0,0,0.18),-6px_-6px_#ffffff]">
            ...
            <button
              onClick={async () => {
                const token = localStorage.getItem("access_token");
                const res = await fetch("/api/store/template", {
                  headers: { Authorization: `Bearer ${token}` },
                });
                if (!res.ok) {
